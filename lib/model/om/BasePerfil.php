...
abstract class BasePerfil extends BaseObject  implements Persistent {
	...
	public function setCodigo($v)
	{
		...
		if ($this->codigo !== $v) {
			$this->codigo = $v;
			$this->modifiedColumns[] = PerfilPeer::CODIGO;
		}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PerfilPeer::CODIGO)) $criteria->add(PerfilPeer::CODIGO, $this->codigo);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PerfilPeer::CODIGO, $this->codigo);
		...
		if ($this->collUsuarios === null) {
			if ($this->isNew()) {
			   $this->collUsuarios = array();
				...
				$criteria->add(UsuarioPeer::PERFIL, $this->codigo);
	...
	public function addUsuario(Usuario $l)
	{
		...
		if (!in_array($l, $this->collUsuarios, true)) { // only add it if the **same** object is not already associated
			...
			$l->setPerfilRelatedByPerfil($this);
		}
	}
	 ...
	 */
	public function getUsuariosJoinPersonaRelatedByPersona($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PerfilPeer::DATABASE_NAME);
		}
		...
		if ($this->collUsuarios === null) {
			if ($this->isNew()) {
				...
				$this->collUsuarios = UsuarioPeer::doSelectJoinPersonaRelatedByPersona($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
