...
abstract class BaseVentaCuniaComercial extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array ProgramacionCuna[] Collection to store aggregation of ProgramacionCuna objects.
	 */
	protected $collProgramacionCunas;

	/**
	 * @var        Criteria The criteria used to select the current contents of collProgramacionCunas.
	 */
	private $lastProgramacionCunaCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->aVentaRelatedByVenta = null;
			$this->aCuniaComercialRelatedByCuniaComercial = null;
			$this->collProgramacionCunas = null;
			$this->lastProgramacionCunaCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collProgramacionCunas !== null) {
				foreach ($this->collProgramacionCunas as $referrerFK) {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getProgramacionCunas() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addProgramacionCuna($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collProgramacionCunas collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addProgramacionCunas()
	 */
	public function clearProgramacionCunas()
	{
		$this->collProgramacionCunas = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collProgramacionCunas collection (array).
	 *
	 * By default this just sets the collProgramacionCunas collection to an empty array (like clearcollProgramacionCunas());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initProgramacionCunas()
	{
		$this->collProgramacionCunas = array();
	}

	/**
	 * Gets an array of ProgramacionCuna objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this VentaCuniaComercial has previously been saved, it will retrieve
	 * related ProgramacionCunas from storage. If this VentaCuniaComercial is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array ProgramacionCuna[]
	 * @throws     PropelException
	 */
	public function getProgramacionCunas($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(VentaCuniaComercialPeer::DATABASE_NAME);
		}
		...
		if ($this->collProgramacionCunas === null) {
			if ($this->isNew()) {
			   $this->collProgramacionCunas = array();
				...
				$this->collProgramacionCunas = ProgramacionCunaPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				ProgramacionCunaPeer::addSelectColumns($criteria);
				if (!isset($this->lastProgramacionCunaCriteria) || !$this->lastProgramacionCunaCriteria->equals($criteria)) {
					$this->collProgramacionCunas = ProgramacionCunaPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastProgramacionCunaCriteria = $criteria;
		return $this->collProgramacionCunas;
	}

	/**
	 * Returns the number of related ProgramacionCuna objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related ProgramacionCuna objects.
	 * @throws     PropelException
	 */
	public function countProgramacionCunas(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(VentaCuniaComercialPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collProgramacionCunas === null) {
			if ($this->isNew()) {
				...
				$count = ProgramacionCunaPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastProgramacionCunaCriteria) || !$this->lastProgramacionCunaCriteria->equals($criteria)) {
					...
					$count = count($this->collProgramacionCunas);
				}
			} else {
				$count = count($this->collProgramacionCunas);
			}
	...
	/**
	 * Method called to associate a ProgramacionCuna object to this object
	 * through the ProgramacionCuna foreign key attribute.
	 *
	 * @param      ProgramacionCuna $l ProgramacionCuna
	 * @return     void
	 * @throws     PropelException
	 */
	public function addProgramacionCuna(ProgramacionCuna $l)
	{
		if ($this->collProgramacionCunas === null) {
			$this->initProgramacionCunas();
		}
		if (!in_array($l, $this->collProgramacionCunas, true)) { // only add it if the **same** object is not already associated
			array_push($this->collProgramacionCunas, $l);
			$l->setVentaCuniaComercial($this);
		}
	}
	...
	{
		if ($deep) {
			if ($this->collProgramacionCunas) {
				foreach ((array) $this->collProgramacionCunas as $o) {
